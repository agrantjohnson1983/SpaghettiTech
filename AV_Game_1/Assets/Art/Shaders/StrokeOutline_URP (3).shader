Shader "Custom/StrokeOutline_URP"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // When width is ~0 (highlight off), skip this pass entirely instead
                // of letting a zero-extrusion backface sit at the same depth as the
                // front face — that would z-fight and flicker rather than vanish cleanly.
                clip(_OutlineWidth - 0.0005);
        ...
        Pass
        {
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                // Additive emission — this is what sObjectHighlighter's Tint mode drives
                // via MaterialPropertyBlock.SetColor("_EmissionColor", ...). Black (default)
                // adds nothing, so objects look unchanged until highlighted.
