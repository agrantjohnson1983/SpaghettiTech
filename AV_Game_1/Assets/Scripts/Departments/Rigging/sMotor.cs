...
public class sMotor : sRigGear
{
    ...
    // Used by sRiggingManager to gate gear (lights, etc.) that should
    // only be mountable once the truss is raised enough to work on.
    public bool IsAtOrAboveWorkingHeight
    {
        get
        {
            return currentStage != eMotorRaiseStage.AtGround;
        }
    }
    ...
    void Start()
    {
        ...
        sRiggingManager.riggingMangerGlobal.motorList.Add(this);
    }
    ...
    IEnumerator MotorRaiseMovement(float _targetHeight, eMotorRaiseStage _resultingStage)
    {
        ...
        // Let the manager know in case this changes whether every
        // motor is now at or above working height - light setup spots
        // show/hide based on that.
        sRiggingManager.riggingMangerGlobal.OnMotorStageChanged();
    }
